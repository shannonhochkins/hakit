/* ------------------------------------------------------------------
  SURFACE (dynamic) — adaptive sRGB mixing schedule
  Steps determined by SWATCH_COUNT constant.
...
function stepProgress(i: number, count: number): number {
  ...
  const eased = t < easing ? Math.pow(t / easing, 0.85) * easing : easing + (t - easing) * 0.4;
...
export function makeSurfaceSwatches(
  ...
  for (let i = 0; i < count; i++) {
    ...
    const isPastHalfWay = i >= Math.floor(count / 2);
    const factor = stepProgress(i, count) * (isPastHalfWay ? i * 0.3 : 1); // 0..1 curve
    let c: ColorInstance;
    if (i === 0) {
