...
export function PageForm({ mode = 'new', dashboardId, pageId, isOpen, onClose, onSuccess }: PageFormProps) {
  ...
  const validateForm = useCallback(() => {
    ...
    const existingPageWithName = dashboard.pages.find(
      p => p.name.toLowerCase() === name.trim().toLowerCase() && p.id !== pageId
    );
    ...
    if (path.length === 0) {
      ...
      if (!valid) {
        ...
        const existingPageWithPath = dashboard.pages.find(
          p => nameToPath(p.name) === path && p.id !== pageId
        );
  ...
  return (
    <Modal open={isOpen} onClose={handleClose} title={<>
      ...
      <form onSubmit={handleSubmit} style={{
        ...
        <FieldGroup>
          <FieldLabel label="Page Name" description="The name of the page." />
          <InputField
            id="page-name"
            value={name}
        ...
        <FieldGroup>
          <FieldLabel label="Page Path" description="The path of the page underneath the dashboard." />
          <InputField
            id="page-path"
            value={path}
            onChange={(e) => {
            ...
            slotProps={{
              ...
              input: {
                startAdornment: dashboard && (
                  <CompactInputAdornment position="start">
                    {dashboard.path}/
                  </CompactInputAdornment>
                ),
              },
            }}
          />
        ...
        <FieldGroup>
          <FieldLabel label="Thumbnail (optional)" description="Upload an image thumbnail for this page." />
          <ImageUpload
            value={thumbnail}
