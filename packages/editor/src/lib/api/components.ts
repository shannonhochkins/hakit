...
export async function getUserRepositories(toastMessage?: ToastMessages): Promise<UserRepositoryWithDetails[]> {
  const response = await callApi(api.components['user-repositories'].$get(), toastMessage);
...
export async function connectRepository(repositoryId: string, versionId: string, toastMessage?: ToastMessages): Promise<UserRepositoryAPI> {
  const response = await callApi(
    api.components['user-repositories'].$post({
      json: {
        repositoryId,
        versionId,
      },
...
export async function updateUserRepositoryVersion(
  ...
  return await callApi(
    api.components['user-repositories'][':userRepoId'].$put({
...
export async function disconnectRepository(userRepositoryId: string, toastMessage?: ToastMessages): Promise<{ message: string }> {
  return await callApi(
    api.components['user-repositories'][':userRepoId'].$delete({
      param: { userRepoId: userRepositoryId },
    }),
    toastMessage
  );
...
export async function toggleComponentStatus(
  ...
  return await callApi(
    api.components['user-repositories'][':userRepoId'].components[':componentName'].toggle.$put({
...
export async function installRepositoryFromGithub(
  ...
  const response = await fetch('/api/components/install/from-github', {
    method: 'POST',
    headers: {
  ...
  if (toastMessage?.success) {
    // Success toast will be handled by calling component
  }
}
