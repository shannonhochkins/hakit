 ...
 */
function convertAstNodeToValue(node: t.Node): any {
  switch (node.type) {
    ...
    case 'ObjectExpression': {
      ...
      for (const prop of node.properties) {
        if (prop.type === 'ObjectProperty') {
          ...
          console.error(`Unsupported property type: ${prop.type} on key ${prop.key}`);
        }
      }
      return obj;
    }
  ...
  traverse(ast, {
    ExportNamedDeclaration(path) {
      ...
      for (const spec of specifiers) {
        ...
        if (t.isExportSpecifier(spec) && t.isIdentifier(spec.exported) && spec.exported.name === 'default') {
  ...
  if (defaultLocalName) {
    traverse(ast, {
      VariableDeclarator(path) {
        ...
        if (t.isIdentifier(varId) && varId.name === defaultLocalName && t.isObjectExpression(init)) {
          // Bingo
